...
def load_and_clean_data():
    try:
        df = pd.read_excel('JLW-JSI.xlsx', sheet_name="Sheet1")
...
def get_chart_data():
    ...
    def calculate_risk_matrix():
        # Define impact and likelihood mappings based on budget utilization
        risk_matrix = {
        ...
        for _, row in filtered_df.iterrows():
            ...
            variance_pct = abs(row['Budget Var Amount'] / budget * 100) if budget != 0 else 0
    ...
    risk_matrix_data = calculate_risk_matrix()

    return jsonify({
         'risk_distribution': {
            'matrix': risk_matrix_data,
            'labels': list(risk_matrix_data.keys()),
            'likelihoods': ['Improbable', 'Remote', 'Occasional', 'Probable', 'Frequent']
        },
        'budget_vs_actual': {
